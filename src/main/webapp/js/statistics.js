...
function drawChart() {
  ...
  $.getJSON('/api/admin/statistics/categoryRank', function (dataset1) {
  ...
  $.getJSON('/api/admin/statistics/top', function (dataset2) {

    let fullNameAndQuantitiesArrayChart2 = [['Recipe Name', 'Quantity',],];

    for (let i = 0; i < dataset2.length; i++) {
      let pairFullNameQuantity = dataset2[i];
      let fullNameAndQuantityArray = [pairFullNameQuantity.recipeName,
  ...
  $.getJSON('/api/admin/statistics/recipeCategoryRank', function (dataset3) {
